...
namespace color
{
  ...
  INLINE u32 RGBAtoBGRA( u32 color ) { return (MASK_RED & (color << 16)) | (MASK_GREEN & color) | (MASK_BLUE & (color >> 16)) | (MASK_ALPHA & color); }
  ...
  INLINE u32 blend( u32 bottom, u32 top )
  {
    u8 t = u8( (top & MASK_ALPHA) >> 24 );

    //TODO: no clue if this is actually correct 
    return color::interpolate( t, bottom, top );
  }
}